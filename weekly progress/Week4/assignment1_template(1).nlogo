...
globals [total_dirty time locations]
...
to setup-patches
  ...
  set locations []
    ask patches [
    set pcolor blue
    if random 100 < dirt_pct [
      set pcolor black
      set total_dirty total_dirty + 1
      set locations lput (list (pxcor) (pycor)) locations
    ]
    ]
end
...
to update-beliefs
 ...
 ask vacuums [
 if time = 0 [
  set beliefs locations
   set beliefs sort-by [  sqrt(((item 0 ?1 - pxcor) *  (item 0 ?1 - pxcor))  + ((item 1 ?1 - pycor) * (item 1 ?1 - pycor))) < sqrt(((item 0 ?2 - pxcor) *  (item 0 ?2 - pxcor))  + ((item 1 ?2 - pycor) * (item 1 ?2 - pycor)))] beliefs
 ]
 ]
end
...
to execute-actions
  ...
  ask vacuums [
    ...
    forward 1
    if pxcor = item 0 intention and pycor = item 1 intention [
     set pcolor blue
